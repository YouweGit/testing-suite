...
## [Unreleased]
### Added
- A commented Pimcore select option example path to phpstan.neon template, which can be enabled to avoid `trait.unused` errors.
...
## [3.2.1]
### Fixed
- Drupal: Faulty `phpmd.xml` include path.
- Drupal: Actually require Drupal PHPCS coding standards.
- Drupal: Update `phpstan.neon` with example for a module.
